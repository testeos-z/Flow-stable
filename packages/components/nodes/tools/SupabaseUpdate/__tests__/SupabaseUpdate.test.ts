const { nodeClass: SupabaseUpdate } = require('../SupabaseUpdate')
...
describe('SupabaseUpdate node', () => {
    it('should return an UpdateTool instance from init()', async () => {
        const node = new SupabaseUpdate()
        const nodeData = {
        ...
        expect(result).toBeInstanceOf(UpdateTool)
    })
    ...
    it('should set correct label, name, type, and category', () => {
        const node = new SupabaseUpdate()
        expect(node.label).toBe('Supabase Update')
        expect(node.name).toBe('supabaseUpdate')
        expect(node.type).toBe('SupabaseUpdate')
