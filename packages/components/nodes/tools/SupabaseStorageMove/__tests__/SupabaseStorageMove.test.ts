const { nodeClass: SupabaseStorageMove } = require('../SupabaseStorageMove')
...
describe('SupabaseStorageMove node', () => {
    it('should return a MoveTool instance from init()', async () => {
        const node = new SupabaseStorageMove()
        const nodeData = {
        ...
        expect(result).toBeInstanceOf(MoveTool)
    })
})
