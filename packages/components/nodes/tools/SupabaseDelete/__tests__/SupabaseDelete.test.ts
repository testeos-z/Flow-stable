const { nodeClass: SupabaseDelete } = require('../SupabaseDelete')
...
describe('SupabaseDelete node', () => {
    it('should return a DeleteTool instance from init()', async () => {
        const node = new SupabaseDelete()
        const nodeData = {
        ...
        expect(result).toBeInstanceOf(DeleteTool)
    })
    ...
    it('should set correct label, name, type, and category', () => {
        const node = new SupabaseDelete()
        expect(node.label).toBe('Supabase Delete')
        expect(node.name).toBe('supabaseDelete')
        expect(node.type).toBe('SupabaseDelete')
